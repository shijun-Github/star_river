...
# 创建蓝图实例
blueprint_video = Blueprint('video', __name__, url_prefix='/')  # 注意这里的 url_prefix 在 app.py 中会再次被覆盖
...
@blueprint_video.route("/video/drama/square", methods=["POST", "GET"])
...
@blueprint_video.route("/video/search/func_get_video_series_info_by_item_id", methods=["POST", "GET"])
...
@blueprint_video.route("/video/search/func_search_drama_by_keyword", methods=["POST", "GET"])
