...
class NetEaseWebApi(HttpRequest):
    ...
    def all_playlist(self, cat='全部歌单', types='all', offset=0, index=1):
        ...
        url = 'http://music.163.com/api/playlist/list?index=%d' % index
        html = self.httpRequest(url, method='GET', cookies=self.cookies)
        print(html)
...
if __name__ == '__main__':
    ...
    """
    net_ease = NetEaseWebApi()
    # help(net_ease)
    # print(net_ease.all_playlist(cat='华语', types='all', offset=3, index=1))
    # print(len(net_ease.all_playlist(cat='华语', types='all', offset=3, index=1)))
    # print(net_ease.all_playlist(cat='华语', types='all', offset=3, index=2))
    print(net_ease.details_playlist('12696691965'))
